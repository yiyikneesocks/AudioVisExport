...
> **面向后续 AI 协作者**：本文档是工程交接文档 —— 架构 / 模块 / 参数表 / 构建 / 已知限制 / 版本映射。
> **⚠️ 任何改动都必须遵守的硬规则（工作流闸门 · 纪律红线 · 发版 checklist · INBOX 三文件 · Python 环境 · 编码/git 陷阱 · 参考优先原则）
> 一律在工程根 `AGENTS.md`（OpenCode 每次会话自动注入，本文档不再重复）**。
> **接手快速上手**：
> 1. `AGENTS.md`（若你的工具未自动注入则手读一遍）—— 硬规则
> 2. `docs/PLAN.md`「当前状态」→ 30 秒知道项目推进到哪一步
> 3. 本文档按需查阅（§1-§5 概述/结构/模块/构建，§6-§9 决策/FAQ/参数表/控件映射）
> 4. `docs/HISTORY.md` §2 版本索引 → `docs/history/<系列>/<版本>.md`（为什么这样做、踩过什么坑）
>
> **何时查什么（速查表）**：
>
> | 你想知道… | 去哪查 |
...
> | 某功能为什么这样做 | `docs/history/<系列>/<版本>.md`（索引见 `docs/HISTORY.md` §2） |
...
> 注意：你后续推进任何操作以后都要按下方「文档地图」分工更新对应文档——**任务进展写 `PLAN.md`，发版写 `docs/history/<系列>/<版本>.md`（并在 `HISTORY.md` §2 索引加行）+ `RELEASE_NOTES.md` + 本文档版本映射表**。先读相关文档，和 GUI 源码实际内容交叉比对，避免“把推断当事实写入文档”。
>
...
>
> **文档地图（按需读取省上下文；硬规则见 `AGENTS.md`）**：
> - 工程根 `AGENTS.md` = **每次会话强制注入的协作硬规则**（闸门/纪律/发版/INBOX/环境/git 陷阱/参考优先）
> - 本文档 = 架构与设计参考（§1-§5 概述/结构/模块/构建，§6-§9 决策/FAQ/参数表/控件映射，§5.9/§5.10 崩溃转储与 Python venv 详录）
> - `docs/PLAN.md` 滚动当前计划；`docs/HISTORY.md` §2 索引 → `docs/history/<系列>/<版本>.md`；`docs/ROADMAP.md` 长期
> - **不再有 `PLAN_vX.Y.Z.md`**（v0.5.6 起）：历史归档统一走 `docs/history/`
> - `docs/RELEASE_NOTES.md` 用户视角公告；`docs/GUI_GUIDE.md` GUI 手册
> - `docs/inbox/` 三件套（**已 gitignore**，异步协议 + 两条分隔线规则见 `AGENTS.md §3`）
...
- **交付形态（v0.5.0 起）**：**WSL 内交叉编译原生 Windows .exe**（Clang 18 MSVC ABI + xwin Windows SDK），
  ...
  工具链与日常命令见 §5「构建」中「Windows 交叉编译」小节；当年 v0.5.0 实施记录见 `docs/history/v0.5/v0.5.0.md`。
...
> **通用化 + 按工程隔离版（现行）**：`docs/GITHUB_PUSH.md` + `scripts/ghpush.sh`；机器级强制由全局 shell helper `~/.config/git/git-credential-perrepo` 兜底（见 §0.5）。要点：**每仓库用独立凭据文件**
