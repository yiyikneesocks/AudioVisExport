...
> 每轮：读文档1 → 新条目（不在指纹表者）备份进文档2「🔄进行中」→ 干活 → 移入「✅」→ 清理按状态闸门删文档1对应行并在文档3+聊天告知删了哪几号。
> **每完成一条任务立刻重读文档1**（防漏 `!!` 加急）；连做好几条后暂停问用户是否推送。
> **任务排序规则（用户 #10，2026-09-09）**：加急(`!!`) > 无争议/实现简单/不需要用户手动验证就能自证的 > 需用户实测确认的 > 需拍板的设计项（列计划）。被"缺用户测试结果"卡住的跳过不阻塞。
