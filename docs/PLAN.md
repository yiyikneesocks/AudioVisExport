...
> **📥 收件箱三文件协议（防读写抢占，务必遵守）**：
> `docs/INBOX.md`=**文档1 用户输入箱**（用户独占写，**AI 永不 write/edit 此文件**）；
> `docs/INBOX_WORKLOG.md`=**文档2 AI 台账**（接手前先复制条目做备份、记「已处理指纹」去重、做完归档+commit）；
> `docs/INBOX_REPLY.md`=**文档3 AI 给用户的信息**（AI 写、用户只读）。
> 每轮：读文档1 → 新条目（不在指纹表者）备份进文档2「🔄进行中」→ 干活 → 移入「✅」→ 要告诉用户的写文档3并在聊天提醒可删文档1对应行。
