...
> **📐 INBOX 两条分隔线规则（v0.5.6 #6，用户设定，AI 不得删这两条线）**：文件里有两条固定线——
> `🗄️ 历史线`（上方=已归档可清理；下方=进行中）与 `⏸️ 暂停线`（上方=本轮推进；下方=暂缓/待议，AI 别动）。
> 读到**暂停线上方**的任务→继续推进；**暂停线下方**→暂缓不动。
> 一轮做完到暂停线汇报时：**先清掉上方已完成的任务行**（走 `tools/inbox.py prune`）；
> 暂时无法解决的→移到**历史线上方**归档；把**暂停线下方**要恢复的任务上移到暂停线上方，保持编号不乱。
