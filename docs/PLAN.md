...
> **本文件是什么**：v0.5.5（下一版）的**滚动计划**——只放"本轮要做什么 + 当前状态"。
> 发版后把结果摘要并入 `docs/history/<系列>/<版本>.md` + `docs/HISTORY.md` §2 索引，然后清空本文件重写。
> **不再另立 `PLAN_vX.Y.Z.md`**（v0.5.6 起，2026-09-12 用户拍板）。长期方向 → `docs/ROADMAP.md`。

> **⚠️ 每次都要遵守的硬规则（工作流闸门 / 纪律红线 / 发版 checklist / INBOX 三文件协议 / 参考优先原则）
> 见工程根 `AGENTS.md`——OpenCode 每次会话自动注入，本文档不再重复。**

---

## 参考：本轮常用触发点速查（细则见 `AGENTS.md`）

| 时刻 | 动作（细则在 `AGENTS.md`） |
|---|---|
| 完成一步 | 更新本文件「当前状态」+ 时间戳（§1 纪律） |
| commit 前 | 若版本号变化：`docs/history/<系列>/<版本>.md` + `HISTORY.md` 索引 + `RELEASE_NOTES.md` + `ARCHITECTURE.md` 版本映射 + 代码版本号（§2） |
| 部署后 | 【验证闸门】停下等用户「全部通过」（§0） |
| 用户下 push 指令 | push + tag + Release（§0） |
| 每完成 INBOX 一条 | 逐字备份进 WORKLOG + `tools/inbox.py prune`（§3） |


> **收件箱三文件协议 + 两条分隔线 + 收尾强制 + 写入安全 + 任务排序** —— 完整规则见工程根 `AGENTS.md §3`（每次会话自动注入，本文档不再重复）。
...
- **边框第 4 轮（用户复报，看图受阻——本模型无图像输入）**：bar/bar-line 顶缘仍"伸到左右边框下面 + 顶/左右不闭合"。
  ...
  修法＝**按样式家族分治**：bar 家族顶缘改回"正上方 rT 窗最小值"向内带（`bufT`），与左/右水平窗同法 →
  三边一条向内轮廓、转角自然直角闭合、无圆帽外伸；line 家族保留居中 strokePath（"line 不用改"）。`overlayCapsFrom`
  仍保证帽不进描边。新增 `vis_mask_test` 用例 16（bar 矩形：顶以上零描边/两侧零外伸/顶左转角闭合/内部露图）。
- **锁定厚度**（用户要求）：删掉旧的 "All edges width" 总滑块，换成 **"Lock edge thickness"** 复选框；勾上后拖动
  任一缘厚度→三边同值联动（纯 UI 态 `lockEdgeWidths`，不进渲染参数）。四套回归全绿、Linux 构建干净。
- **待办：peak cap 双模式 + 厚度/y2k 点线**（用户新功能，进行中）。
...
- **文档结构再整理：新建工程根 `AGENTS.md`（用户主动要求）**。OpenCode 官方 docs 确认 `AGENTS.md` 每次会话自动注入、
  ...
  + #11/#0/#10 + 写入安全/参考优先/git 陷阱/混合换行符 等）全部集中到 `AGENTS.md §0-§7`；
  ARCHITECTURE.md/PLAN.md 相应块删除（-27 行 / -22 行），留指针指向 AGENTS.md。**未动**：本文档 30+ 行的"当前状态"
  本轮工作日志、版本映射表、代码架构细节。四套回归全绿、无代码影响、双端未重建。

- **task1 v2 溯源 + 重做（INBOX r6 task1 复报，用户提示"应等同于 line 模式的 line 当边框"）**：
  前一版"平滑曲面+法向AA带"仍在像素域近似，本质无法达到线模式那种平滑。真正根因是**用了不同的渲染器**：
  线模式=矢量 `g.strokePath(path, PathStrokeType(w, curved, rounded))`（亚像素中心线·JUCE 矢量 AA·
  天然恒定法向宽·圆角连接），而我此前把边框做成像素距离带——中心线量化到像素中心、只在形状内侧、
  alpha 通道只能 1px AA、跨 gap 桥连。→ **本次把顶缘彻底改成 strokePath**：从 base alpha 抽**亚像素顶中心线**
  （对 128 阈值线性内插），按连续段(gap 断开)构造 Path，用 `PathStrokeType(rT, curved, rounded)` 居中描一遍；
  perBar 时按颜色变化切段各描各的。这样"边框=线条"从原理上等价，锯齿消失。左右侧边维持窗式（竖直边、无此问题）。
  `vis_mask_test` case 11 重定标（flat 9 / 45° 12 = √2·rT，均 ok）+ case 12 简化为覆盖率+AA 计数（300/300 覆盖、
  AA px 存在）。四套回归全绿、双端干净、部署 `AudioVisGUI_09141912.exe`。详细报告见 REPLY。

- **修 Mask 描边大量锯齿（INBOX r6 task1）**：上一版"法向等宽"用**逐列竖直窗最小值 + 逐列斜率算窗宽**，
  在锯齿状顶面（频谱逐 bin 台阶）上窗宽逐列抖动 → 描边内缘大量锯齿。改成**连续平滑顶面 f(x) + 逐像素法向距离
  dperp + [0,rT] 带内两端线性渐入渐出(≈1.5px AA)** 解析描边，斜率只取平滑面低频(中心差分窗±2) → 沿法向恒定厚度、
  边缘抗锯齿、无台阶抖动。左右侧边维持原窗式（竖直边、本就无锯齿）。`vis_mask_test` 用例 11（平8/斜45≈11=√2·rT）
  + 新增用例 12（正弦顶面：法向宽度跨曲线恒定 spread<1.9、254/294 列有 AA 中间灰阶、并落 PNG 供肉眼核）。
  描边算法解释见 REPLY。四套回归全绿、双端干净、部署 `AudioVisGUI_09130953.exe`。

- **INBOX r6 task3 已确认完成并清理**：多选移动测试没问题（`b459fb1` 组拖拽 + `c66d5a2` 双向列表）。
