...
## v0.5.5 — 2026-09-15（边框描边大改 + 峰帽外观可调 + 多选联动 + 默认值文件）

> 正式版。重点：频谱**蒙版边框**全面重做（更准、更平滑、与峰帽解耦）、**峰帽外观可调**、
> **画布与图层列表双向多选**、**默认值文件 config.json**，并修掉一批崩溃/稳健性问题。
> 开发视角完整记录见 `docs/history/v0.5/v0.5.5.md`。

### 新功能
- **蒙版边框（描边）体系**：上/左/右三边各自"开关 + 厚度 + 透明度"（去掉无意义的底边）；
  边框改为一套开关组（总开关→实时变色→逐柱变色）；描边**厚度沿曲线法向等宽**、边缘**抗锯齿**，
  斜线不再变细也不再有锯齿；bar/bar-line 边框沿柱体**向内闭合成一条轮廓**（转角直角相接、不断开、不外溢）。
- **逐柱变色描边修复**：perBar 之前"所有柱一个色"（大小写不一致 bug）现已正常，每柱按各自可视区取色。
- **峰帽（peak caps）外观**：新增独立 **"Peak cap thickness"** 厚度旋钮（与边框宽度互不影响）；
  y2k 线族峰线可选 **点线 / 完整曲线**（点直径=该厚度，点间距自动留出、不会粘连）；
  有边框时可选峰帽 **穿透（透出蒙版图）/ 边框色样式** 两模式。
- **多选联动**：画布 `Ctrl+A` 全选、`Ctrl+点击` 增删、批量删除、整体拖动（多选时禁旋转/拉伸）；
  图层列表同样支持 `Ctrl/Shift` 多选，且与画布高亮**双向同步**。
- **默认值文件**：exe 目录放 `config.json` 即可自定义启动默认值；面板 "Set as startup default" 一键
  把当前所有参数存为默认；文件缺项自动用内置默认、新增参数无需维护该文件。
- **边框阴影暂时下线**（保留代码），阴影滑块置灰。

### 修复 / 稳健性
- 修复 Windows 下"顶边框完全不显示 / line 模式整条无边框"（位图写锁冲突）。
- 修复边框大量锯齿、边框盖到峰帽上、全选后拖不动整组、图层列表全黑、CJK 拖放文件名乱码等。
- 消除描边越界访问崩溃；`contain` 几何零维放大保护。

### 升级注意事项
- 旧项目配置里若含**底边框**或**阴影**参数仍可加载，但底边已移除、阴影当前不生效。
- `Ctrl+Z` 本版**暂不可用**（撤销语义在重做，见 ROADMAP）。
